...
sudo docker exec -t pg psql -U postgres -c "DROP DATABASE phit WITH (FORCE);"

# Create PHIT database and Synthea schema
echo "Creating PHIT database..."
sudo docker exec -t pg psql -U postgres -c "CREATE DATABASE phit;"
sudo docker exec -t pg psql -U postgres -d phit -c "CREATE SCHEMA IF NOT EXISTS synthea;"
...
sudo docker exec -t pg psql -U postgres -d phit -f "/home/synthea_tables.sql"

# Generate Synthea data
read -p "Enter desired population value: " population
sudo docker exec -t synthea bash -c "cd synthea/ && ./run_synthea -p ${population} --exporter.csv.export true"
...
for FILE in "${array[@]}"; do
  sudo docker exec -t pg psql -U postgres -d phit -c "SET schema 'synthea';" -c "COPY ${FILE} FROM '/home/${FILE}.csv' WITH (format csv, header true, delimiter ',');"
done
...
sudo docker exec -t pg psql -U postgres -d phit -f "/home/synthea_views.sql"
...
sudo docker exec -t pg psql -U postgres -d phit -c "CREATE SCHEMA IF NOT EXISTS acs;"
...
sudo docker exec -t pg psql -U postgres -d phit -f "/home/api_views.sql"
sudo docker exec -t pg psql -U postgres -d phit -c "SET schema 'acs';" -c "COPY population_change FROM '/home/population_data.csv' WITH (format csv, header true, delimiter ',');"
